...
function s.initial_effect(c)
    --Invocation Spéciale (Hard Once Per Turn)
    local e1=Effect.CreateEffect(c)
    e1:SetType(EFFECT_TYPE_FIELD)
    e1:SetCode(EFFECT_SPSUMMON_PROC)
    e1:SetProperty(EFFECT_FLAG_UNCOPYABLE)
    e1:SetRange(LOCATION_HAND)
    e1:SetCountLimit(1,id,EFFECT_COUNT_CODE_OATH) -- Ajout de la limite
    e1:SetCondition(s.spcon)
    e1:SetValue(1)
    c:RegisterEffect(e1)
    
    --Changement de Niveau (Once Per Turn)
    ...
    e2:SetCountLimit(1,{id,1}) -- Ajout de la limite
...
-- Filtre pour vérifier la présence d'un autre Gagaga
function s.filter(c)
    return c:IsFaceup() and c:IsSetCard(SET_GAGAGA) and c:GetCode()~=id
end

-- Condition d'Invocation Spéciale
...
-- Condition : Doit être Invoqué Spécialement par son propre effet (Value 1)
function s.lvcon(e,tp,eg,ep,ev,re,r,rp)
    return e:GetHandler():GetSummonType()==SUMMON_TYPE_SPECIAL+1
end

-- Coût : Pas de Battle Phase ce tour
...
-- Filtre pour la cible du niveau
function s.lvfilter(c,lv)
    local clv=c:GetLevel()
    return c:IsFaceup() and c:IsSetCard(SET_GAGAGA) and clv>0 and clv~=lv
end
